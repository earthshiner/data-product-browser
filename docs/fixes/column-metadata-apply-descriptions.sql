 ...
 * has a row for that column. Existing non-null descriptions are NOT
 * overwritten — change the WHERE clause if you want to refresh them.
 *
 * Run order:
 *   A. Preview which rows will change.
 *   B. Apply UPDATEs.
 *   C. Verify residual gaps (rows still missing a description).
 * ------------------------------------------------------------------------ */

/* --- A. Preview rows that will get a description -------------------- */
SELECT COUNT(*) AS rows_to_update
FROM   CallCentre_SEM_STD_T.column_metadata m
WHERE  (m.business_description IS NULL OR TRIM(m.business_description) = '')
